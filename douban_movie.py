...
def get_li(doc):
    ...
    for i in ol.find_all('li'):
        ...
        if info := i.find('span', attrs={'class': 'inq'}):
    ...
    if page := soup.find('span', attrs={'class': 'next'}).find('a'):
...
def main():
    ...
    for (i, m, o, p) in zip(name, star_con, score, info):
        col_A = f'A{name.index(i) + 1}'
        col_B = f'B{name.index(i) + 1}'
        col_C = f'C{name.index(i) + 1}'
        col_D = f'D{name.index(i) + 1}'
        ws1[col_A] = i
        ws1[col_B] = m
        ws1[col_C] = o
        ws1[col_D] = p
